            ...
            for return_line in self.product_return_moves:
                ...
                if return_line.quantity > 0.0:
                    if return_line.move_id not in moves:
                        ...
                        move_orig_to_link |= return_line.move_id \
                            .mapped('move_dest_ids').filtered(lambda m: m.state != 'cancel') \
                            .mapped('move_orig_ids').filtered(lambda m: m.state != 'cancel')
                        ...
                        r.write(vals)
                        if return_line.move_id.state not in ('cancel', 'done'):
