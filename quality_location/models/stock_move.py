...
class StockMove(models.Model):
    ...
    def _onchange_lot_domain_id(self):
        if self.product_id:
            domain = {'restrict_lot_id': [('product_id', '=', self.product_id.id), ('quant_ids.location_id', '=', self.location_id.id)]}
            return {'domain': domain}

        if self.restrict_lot_id and not self.product_id:
            self.product_id = self.restrict_lot_id.product_id.id
            self.product_uom_qty = sum(self.restrict_lot_id.quant_ids.filtered(lambda quant: quant.location_id.id == self.location_id.id).mapped('quantity'))
