    ...
    def _compute_show_validate(self):
        for picking in self:
            if picking.state == 'assigned' and picking.approve and self.location_id in self.env.user.stock_location_ids and self.location_dest_id in self.env.user.stock_location_ids:
                if (
                        self.location_id.stock_usage == 'qrtin' or self.location_dest_id.stock_usage == 'qrtin') and self.env.user.has_group(
                        'quality_location.group_stock_approve') and not picking.batch and self.picking_type_id.code == 'internal':
                    picking.show_validate = True
                elif (
                        self.location_id.stock_usage == 'qrtin' or self.location_dest_id.stock_usage == 'qrtin') and self.env.user.has_group(
                        'stock.group_stock_manager') and not self.env.user.has_group(
                        'quality_location.group_stock_approve') and not picking.batch and self.picking_type_id.code != 'internal':
                    picking.show_validate = True
                elif (
                        self.location_id.stock_usage == 'qrtin' or self.location_dest_id.stock_usage == 'qrtin') and self.env.user.has_group(
                        'stock.group_stock_manager') and not self.env.user.has_group(
                        'quality_location.group_stock_approve') and picking.batch:
                    picking.show_validate = True

                elif self.location_dest_id.stock_usage != 'qrtin' and self.location_id.stock_usage != 'qrtin' and self.env.user.has_group(
            ...
            if picking.state == 'assigned' and not picking.approve and self.location_id in self.env.user.stock_location_ids and self.location_dest_id in self.env.user.stock_location_ids:
                if (
                    ...
                    picking.show_cofirm = True
                        ...
                        self.location_id.stock_usage == 'qrtin' or self.location_dest_id.stock_usage == 'qrtin') and self.env.user.has_group(
                        'stock.group_stock_user') and not self.env.user.has_group(
                        'quality_location.group_stock_approve') and picking.batch:
                    picking.show_cofirm = True
