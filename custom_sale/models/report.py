            ...
            for partner in partners:
              if self.state == 'category':
                  invoices = self.env['account.move'].search([
                      ('invoice_date', '>=', rec.from_date),('state', 'not in', ('cancel','draft')), ('invoice_date', '<=', rec.to_date), (
        ...
        for rec in self:
            ...
            if self.category_type:
                ...
                if self.return_inv:
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', '=', 'out_refund'), ('state', 'not in', ('cancel','draft')),('partner_id', 'in', partners),('cust_categ_id.category_type','=',self.category_type.category_type)
                    ])
                ...
                if self.invoice_inv and self.return_inv:
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', 'in', ('out_refund', 'out_invoice')), ('state', 'not in', ('cancel','draft')),('cust_categ_id.category_type','=',self.category_type.category_type),('partner_id', 'in', partners)
                    ])
                ...
                if self.invoice_inv:
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', '=', 'out_invoice'), ('state', 'not in', ('cancel','draft')),('partner_id', 'in', partners)
                    ])
                ...
                if self.return_inv:
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', '=', 'out_refund'), ('state', 'not in', ('cancel','draft')),('partner_id', 'in', partners)
                    ])
                ...
                if self.invoice_inv and self.return_inv:
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', 'in', ('out_refund', 'out_invoice')),('state', 'not in', ('cancel','draft')),('partner_id', 'in', partners)
                    ])
        ...
        for rec in self:
            ...
            for partner in partners:
                if self.state == 'category':
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', '=', 'out_invoice'), ('state', 'not in', ('cancel','draft')),('partner_id', '=', partner),('cust_categ_id.category_type','=',self.category_type.category_type)])
                    total = 0
                    res = 0
                    status=' '
                    for inv in invoices:
                    ...
                    invoices = self.env['account.move'].search([
                        ('invoice_date', '>=', rec.from_date), ('invoice_date', '<=', rec.to_date), (
                            'move_type', '=', 'out_invoice'),('state', 'not in', ('cancel','draft')),('partner_id', '=', partner)])
                    total = 0
                    res = 0
                    status=''
