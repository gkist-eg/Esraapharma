...
class PaymentRequest(models.Model):
    ...
    def change_inspections(self):
        ...
        for line in self.picking_ids:
            for move in line.move_line_ids:
                    ...
                    done = sum([quant.qty_done for quant in lines])
                    ...
                    done += sum([quant.qty_done for quant in request])
                    if lines:
    ...
    def _onchange_order_id(self):
        ...
        if self.order_id and not self.picking_ids:
            don_pickings = []
            pickings = []
            orders = self.search([('order_id', '=', self.order_id.id)])
            if orders:
                for line in orders:
                    for ins in line.picking_ids:
                        don_pickings.append(ins.id)
