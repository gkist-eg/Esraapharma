...
class SafeStatement(models.TransientModel):
    _name = 'safe.statement'

    type_cash = fields.Selection([
        ('Partner', 'Partner')
    ], store=True, default='Partner', invisible=True)
    ...
    journal_id = fields.Many2one('account.journal', 'Save', store=True, track_visibility='onchange', required=True,
    ...
    line_ids = fields.One2many('safe.statement.line', 'wizard_id', required=True, ondelete='cascade')

    def print_pdf_safe(self):
        line_ids = []
        # Unlink All one2many Line Ids from same wizard
        for wizard_id in self.env['safe.statement.line'].search([('wizard_id', '=', self.id)]):
            ...
            if wizard.type_cash == 'Partner':
                ...
                invoice_objs = self.env['account.move.line'].search(
                    [('date', '>=', wizard.date_from),
                     ('date', '<=', wizard.date_to), ('account_id', '=', wizard.journal_id.default_account_id.id),
                     ('credit', '=', 0.0)
                     ])
        ...
        return {
            ...
            'report_name': 'report.safe_statement_report',
            'report_type': 'qweb-html',
            'report_file': 'report.safe_statement_report',
            'name': 'safe_statement_report',
...
class safeStatementLine(models.TransientModel):
    _name = 'safe.statement.line'

    wizard_id = fields.Many2one('safe.statement', required=True, ondelete='cascade')
    ...
    amount_employee = fields.Char(string="Amount Paid")
